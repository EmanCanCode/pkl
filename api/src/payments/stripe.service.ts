...
export class StripeService implements OnModuleInit {
  ...
  async onModuleInit() {
    if (!process.env.STRIPE_SECRET_KEY) {
      this.logger.warn(
        "Skipping Stripe product setup — no secret key configured",
      );
      return;
    }
  ...
  private async ensureMembershipProduct(): Promise<void> {
    ...
    try {
      const existing = await this.findProductByMetadata(
        "pkl_annual_membership",
      );
      if (existing) {
        ...
        if (prices.data.length > 0) {
          ...
          this.logger.log(
            `Membership price (existing): ${this.membershipPriceId}`,
          );
          return;
        }
      }
      ...
      const product = await this.stripe.products.create({
        ...
        description:
          "Annual membership required to register for PKL Pathway Series events. Grants one full year of event access.",
      ...
      this.logger.log(
        `Created membership product ${product.id} → price ${price.id} ($10/year)`,
      );
    } catch (error: any) {
      this.logger.error(
        `Failed to set up membership product: ${error.message}`,
      );
    }
  }
  ...
  getMembershipPriceId(): string {
    if (!this.membershipPriceId) {
      throw new Error(
        "Membership price not configured — check Stripe setup and STRIPE_SECRET_KEY",
      );
    }
  ...
  async createEventFeeCheckoutSession(
    customerId: string,
    userId: string,
    eventId: string,
    eventName: string,
